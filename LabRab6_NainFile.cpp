...
public:
	...
	virtual ~Figure();//* деструктор для освобождения контекста устройства
};
...
//*(154-158) Определение деструктора для освобождения контекста устройств                                                           
Figure::~Figure()
{
	ReleaseDC(hWnd, hdc);
}
...
HANDLE hEvents[102]; //* Хэнделы событий для синхронизации потоков
Figure *pF[102];
ForThread Data[102]; //* Данные для потоковых функций для каждого из 9 потоков
int flag = 1; //* Условие работы цикла в потоках

DWORD WINAPI ThreadFun(LPVOID par)
{
	ForThread *pData = (ForThread *)par; // Данные, которые переданы  в потоковую функцию
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		for (int i = 0; i<102; i++)
		{
			...
			Data[i].hEvent = hEvents[i] = CreateEvent(0,false,false,0);//объект для синхронизации потоков(2-TRUE сброс вручную FALSE автосброс;3-свободное(TRUE)занятое (FALSE).
